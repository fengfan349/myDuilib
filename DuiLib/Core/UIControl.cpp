...
void CControlUI::SetScale(bool bScale)
{
    m_bScale = bScale;
}
...
void CControlUI::SetPercent(int nPercent)
	{
		m_nPercent = nPercent;
	}

int CControlUI::GetPercent() const
	{
		return m_nPercent;
	}
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("Scale")) == 0 ) SetScale(_tcscmp(pstrValue, _T("true")) == 0);
	else if( _tcscmp(pstrName, _T("percent")) == 0 ) {
				LPTSTR pstr = NULL;
