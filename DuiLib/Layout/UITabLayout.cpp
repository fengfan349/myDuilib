...
namespace DuiLib
{
	...
	void CTabLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			...
			if (m_bScale)
			{
				CHorizontalLayoutUI* pHorizontalLayout = static_cast<CHorizontalLayoutUI*>(pControl);
				sz.cx = int(nHeaderWidth * (float)pHorizontalLayout->GetPercent() / 100);
				sz.cy = int(nHeaderHeight * (float)pHorizontalLayout->GetPercent() / 100);
				
				CVerticalLayoutUI* pVerticalLayout = static_cast<CVerticalLayoutUI*>(pControl);
				sz.cx = int(nHeaderWidth * (float)pVerticalLayout->GetPercent()/ 100);
				sz.cy = int(nHeaderHeight * (float)pVerticalLayout->GetPercent()/ 100);
				
				CButtonUI* pButton = static_cast<CButtonUI*>(pControl);
				sz.cx = int(nHeaderWidth * (float)pButton->GetPercent() / 100);
				sz.cy = int(nHeaderHeight * (float)pButton->GetPercent() / 100);
				
				COptionUI* pOption = static_cast<COptionUI*>(pControl);
				sz.cx = int(nHeaderWidth * (float)pOption->GetPercent() / 100);
				sz.cy = int(nHeaderHeight * (float)pOption->GetPercent() / 100);
				
				CTabLayoutUI* pTabLayout = static_cast<CTabLayoutUI*>(pControl);
				sz.cx = int(nHeaderWidth * (float)pTabLayout->GetPercent() / 100);
				sz.cy = int(nHeaderHeight * (float)pTabLayout->GetPercent() / 100);
			}
